## 영화 정보 소개 및 좌석 예약 플랫폼 - '무비캐쳐'   
### 개요
- 최신 영화의 정보와 순위를 제공하고 실제 영화관 좌석 예매까지 연결 시켜주는 플랫폼입니다.
- 여러 영화 공공데이터 API를 사용하여 여러 영화정보를 받아오고, CGV의 좌석정보를 크롤링하여 화면에 보여주며 실제 결제모듈을 제외한 모든 예매 사이클을 진행합니다.
### 프로젝트
- 참여인원 : 4명
- 기간 : 2019-12 ~ 2020-01
- 참여도 : 60%
- 담당 역할 : 기획, 설계, 개발, 발표
- 담당 개발 : 메인페이지, 영화정보 페이지, 상세페이지, 네이버 영화리뷰 크롤링, 영화 공공API 사용
### 사용기술
- FE : HTML, CSS, JavaScript, BootStrap
- BE : JAVA(jdk 1.8.0_221), JSP, JSTL, EL
- DB : JDBC, MariaDB(10.4ver)
- Library
  - JSoup (네이버 영화 리뷰 및 별점 크롤링)
  - 영화진흥원 API, themoviedb API를 서로 맞물리게 구현 (중복 영화 핸들링)
